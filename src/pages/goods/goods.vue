...
import AddressPanel from './conponents/AddressPanel.vue'
import ServicePanel from './conponents/ServicePanel.vue'
import GoodsSkeleton from './conponents/GoodsSkeleton.vue'
...
const getGoodsData = async () => {
  const res = await getGoods(query.id)
  ...
  localData.value = {
    ...
    spec_list: res.result.specs.map((v) => ({ list: v.values, name: v.name })),
  }
}
...
<template>
  ...
  <scroll-view scroll-y class="viewport" v-if="!isLoading">
    ...
    <view class="goods">
      ...
      <view class="preview">
        <swiper circular @change="onChange">
          <swiper-item v-for="item in goodsList?.mainPictures" :key="item" @tap="onTapImg(item)">
      ...
      <view class="action">
        <view class="item arrow" @tap="openSku(SkuMode.Both)">
          ...
          <text class="text ellipsis"> {{ skuArrText }} </text>
        </view>
    ...
    <view class="detail panel">
      ...
      <view class="content">
        ...
        <!-- 图片详情 -->
        <image mode="widthFix" v-for="item in goodsList?.details.pictures" :key="item" :src="item">
        </image>
      </view>
    </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="buttons">
      <view @tap="openSku(SkuMode.Cart)" class="addcart"> 加入购物车 </view>
      <view @tap="openSku(SkuMode.Buy)" class="buynow"> 立即购买 </view>
    </view>
  </view>
