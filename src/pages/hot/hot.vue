...
import { getHotRecomend } from '@/services/hot'
...
// 找到type对应相关值
const curMap = hotMap.find((item) => item.type === props.type)

// 设置动态标题
uni.setNavigationBarTitle({ title: curMap!.title })
...
const getHotRecommendData = async () => {
  const res = await getHotRecomend(curMap!.url)
...
const onScrollToLower = async () => {
  ...
  // 根据获取数据得到新页
  const newTypes = res.result.subTypes[activeIndex.value]

  currentTypes.goodsItems.items.push(...newTypes.goodsItems.items)
}
</script>
...
<template>
  <view class="viewport">
    ...
    >
      ...
      <view class="loading-text">{{ item.isFinish ? '已经到底了~' : '正在加载...' }}</view>
    </scroll-view>
  </view>
</template>
