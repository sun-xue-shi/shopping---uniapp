...
// 步进器数据
const onCountChange = (ev: InputNumberBoxEvent) => {
  putMemberCart(ev.index, { count: ev.value })
}

// 修改单选状态
const onSelectedChange = (item: CartItem) => {
  item.selected = !item.selected
  putMemberCart(item.skuId, { selected: item.selected })
}
...
const onSelectAllChange = () => {
  ...
  putMemberCartSelect({ selected: _isSelectAll })
}
...
const selectedCartCount = computed(() => {
  return selectedCart.value.reduce((count, item) => count + item.count, 0)
})
// 选中商品总金额
const selectedCartMoney = computed(() => {
  return selectedCart.value.reduce((sum, item) => sum + item.count * item.nowPrice, 0).toFixed(2)
})
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrollToLower">
    ...
    <template v-if="memberStore.profile">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" :style="{ paddingTop: safeAreaInsets!.bottom + 'px' }">
        <text class="all" :class="{ checked: isSelectAll }" @tap="onSelectAllChange">全选</text>
        <text class="text">合计:</text>
        <text class="amount">{{ selectedCartMoney }}</text>
    ...
    <!-- 猜你喜欢 -->
    <GuessLike ref="guessRef"></GuessLike>
    <!-- 底部占位空盒子 -->
